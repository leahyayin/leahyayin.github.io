...
def get_member_info(connection, username):
    cursor = connection.cursor()
    ...
    cursor.close()
    if res:
        data = {'id': res[0], 'name': res[1], 'username': res[2]}
        print("get_member_info:", data.items())
    else:
        data = None
        
    return data

def update_name(connection, name, member_id):
